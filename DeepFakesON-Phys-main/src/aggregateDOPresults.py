...
min_score = 0.579 #defined in DOP paper
DOP_output_scores = 'deepfake_scores.txt2549651' #output file from DOP to be aggregated
...
# Read the input file
with open(DOP_output_scores, 'r') as file:
    for line in file:
        if "img;score" in line:
            continue
        line = line.strip().replace("\\", "/").split(';')
        video_path = os.path.splitext(line[0])[0]
        video_path = video_path.rsplit('.mp4', 1)[0]
