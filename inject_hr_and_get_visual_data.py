...
def inject_heart_rate(video_path, output_path, target_heart_rate):
    ...
    for i in range(frame_count):
        ...
        if len(face_rects) > 0:
            ...
            roi_hsv[:, :, 2] = np.clip(roi_hsv[:, :, 2] + sinusoidal_signal[i] * skin_mask, 0, 255) # we're not using signal here because we want the negative part of the sinusoidal signal
            ...
            signal_scaled = (np.clip(8*signal / 255 * 255,0,255)).astype(np.uint8) # 8 * signal is used to make the signal more visible
            original_scaled = (original / 255 * 255).astype(np.uint8)
            result_scaled = (result / 255 * 255).astype(np.uint8)

            # Append the scaled images to the corresponding lists
    ...
    # Save videos
    save_as_video(modified_frames, output_path, fps)

    # Save signals as videos
    save_as_video(signal_frames, 'oral/signal.mp4', fps/6)
    save_as_video(original_frames, 'oral/original_with_mask.mp4', fps/6)
    save_as_video(result_frames, 'oral/result.mp4', fps/6)
